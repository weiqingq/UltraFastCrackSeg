...
class isic_loader(Dataset):
    ...
    def __init__(self, path_Data, train = True, Test = False):
        ...
        self.train = train
        if train:
          self.data   = np.load(path_Data+'data/data_train.npy')
          self.mask   = np.load(path_Data+'data/mask_train.npy')
        else:
          if Test:
            self.data   = np.load(path_Data+'data/data_test.npy')
            self.mask   = np.load(path_Data+'data/mask_test.npy')
          else:
            self.data   = np.load(path_Data+'data/data_val.npy')
            self.mask   = np.load(path_Data+'data/mask_val.npy')          
